...
export const taskListReducer = (state = defaultStore, action: TaskListAction): ITaskListState => {
    switch(action.type){
        ...
        case TaskListTypes.FETCH_TASKLIST_UPDATE:
            return {isLoading: false, error: null, taskLists: [...state.taskLists.map((el) => (el.id === action.payload.id ? action.payload : el))]}
        case TaskListTypes.FETCH_TASKLIST_DELETE:
            return {isLoading: false, error: null, taskLists: [...state.taskLists.filter((el) => ( action.payload.indexOf(el.id) === -1 ? true : false ))]}
