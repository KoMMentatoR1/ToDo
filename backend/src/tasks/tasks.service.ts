...
export class TasksService {
  ...
  async getByListId(ListId: number, authorization: string) {
    ...
    if (!tasklist) throw new HttpException('Лист не найден', HttpStatus.NOT_FOUND);

    if(tasklist.UserId !== decoded.id) throw new HttpException('Лист не принадлежит этому пользователю', HttpStatus.NOT_FOUND); //fix

    const task = await this.taskRepository.findAll({
      where: { TaskListId: ListId },
    });

    if (!task) throw new HttpException('Задачи не найдены', HttpStatus.NOT_FOUND);
  ...
  async delete(id: number, taskListId: number) {
    ...
    if (!taskList) throw new HttpException('Лист задач не найден', HttpStatus.NOT_FOUND);
    if(task.TaskListId !== taskListId) throw new HttpException('Задача не принадлежит листу задач', HttpStatus.NOT_FOUND);
    await task.destroy();
    return task;
  }
}
